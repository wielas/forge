...
3. Score every scenario in the frozen feature. A tagged `@real-source` scenario
   remains in the scored acceptance surface: confirm that its step definitions
   exercise the named source and do not silently substitute a fixture.
4. Check reconciliation: do ROADMAP/ADR/AGENTS.md updates in the diff match what
   the code actually changed? Undocumented drift is a finding, not a nitpick.
5. Look for the failure modes no program catches:
   ...
   - **Scope creep the path check cannot see**: a file inside `Touches` that
     ...
     yours.
6. Emit the verdict JSON exactly per the rubric schema, then a ≤10-line human
