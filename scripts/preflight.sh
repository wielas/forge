...
# --- the OTHER half of the question: which skills are ENABLED ---------------
# The loop above answers "can this profile SEE the forge skills". It is
# structurally blind to enable/disable state, and those are different
# questions. Only one of them was ever asked, so roughly eighty bundled
# builtins — sdlc-review, merge-reconciler and three github PR drivers among
# them — sat ENABLED in every lane profile for the entire life of this project
# while this section reported PASS.
#
# skills.disabled is generated by hermes/profiles-bootstrap.sh as the complement
# of skills_allowed_for(), computed from the set installed AT GENERATION TIME.
# Anything installed since — a builtin arriving with a Hermes update, a hub
# install, a project-local skill in a trusted repo — is absent from that list
# and is therefore enabled. This check is what notices; the fix is always the
# same, `./hermes/profiles-bootstrap.sh`.
#
# WARN, NOT FAIL, and that is a judgement worth stating. An unexpected enabled
# skill is methodology-erosion risk, not a run-stopper: the one name here with
# teeth is sdlc-review, and review dispatch only force-loads it on the REVIEW
# lane, which forge's current model (implementation card completes, a
# pre-created child card carries review) never enters. Meanwhile section 4b
# records what a FAIL on a system that is working actually costs — it teaches
# its operator to ignore the gate — and `make preflight` runs after every merge
# to main. This will fire on any host whose profiles predate the widening, which
# is correct and is not a reason to be red.
#
# FORGE_PREFLIGHT_SKILLS_CMD is a TEST SEAM, not an operator knob, kept out of
# the --help header for the same reason as FORGE_MAX_IN_PROGRESS (section 4) and
# FORGE_HERMES_SHIM (section 11): it names the command this check enumerates
# with, invoked exactly as `<cmd> -p <profile> skills list`.
PF_SKILLS_CMD="${FORGE_PREFLIGHT_SKILLS_CMD:-hermes}"
# Presentation only. Every unexpected name is COUNTED; these are the ones named
# out loud first, because a single line carrying eighty bundled builtins is
# noise an operator skims past. They are the ones that can act in forge's own
# layers: a reviewer, a merger, three PR drivers and a browser-recovery loop.
PF_ADJACENT="sdlc-review merge-reconciler github-pr-workflow github-code-review github-issue-to-pr blocked-page-recovery"
# Resolved here rather than by hoisting section 10's FORGE_DIR. Re-ordering a
# block that currently produces live PASSes so a new check can borrow one
# variable is precisely the "a check went quietly blind when something moved"
# risk this repo keeps paying for.
if [ -n "$FORGE_DIR_OPT" ]; then
  PF_FD="$FORGE_DIR_OPT"
elif [ -n "${BASH_SOURCE[0]:-}" ]; then
  PF_FD="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
elif [ -f "./Makefile" ] && [ -d "./skills" ]; then
  PF_FD="$(pwd)"
else
  PF_FD=""
fi
PF_BOOTSTRAP="$PF_FD/hermes/profiles-bootstrap.sh"

if ! command -v "$PF_SKILLS_CMD" >/dev/null 2>&1; then
  warn "cannot enumerate enabled skills: '$PF_SKILLS_CMD' is not executable. A"
  say  "      control that could not run has NOT passed (F65/F66)."
elif [ -z "$PF_FD" ] || [ ! -f "$PF_BOOTSTRAP" ]; then
  warn "cannot read the allowlist: no hermes/profiles-bootstrap.sh under"
  say  "      '${PF_FD:-<unresolved>}'. Re-run from a checkout or pass --forge-dir."
elif ! grep -q '^skills_allowed_for()' "$PF_BOOTSTRAP"; then
  # Asserted SEPARATELY from reading a value, because forge-prejudge's allowlist
  # is legitimately empty: "no case arm" and "the function moved" produce the
  # same empty string, and only one of them is a pass.
  warn "hermes/profiles-bootstrap.sh has no skills_allowed_for() — the allowlist"
  say  "      shape moved and this check cannot tell 'allowed nothing' from 'could"
  say  "      not read'. A parse that went blind is not a pass (F65)."
else
  for pdir in "$HOME"/.hermes/profiles/forge-*/; do
    [ -d "$pdir" ] || continue
    pname="$(basename "$pdir")"
    # Same gate as the loop above: no `skills` toolset, no skills to police.
    grep -qE '^[[:space:]]+- skills$' "$pdir/config.yaml" 2>/dev/null || continue

    _allow="$(sed -n "s/^[[:space:]]*$pname)[[:space:]]*echo \"\([^\"]*\)\";;.*/\1/p" \
              "$PF_BOOTSTRAP" | head -1)"
    # COLUMNS through `env`: the table truncates the Name column to the terminal
    # width, and `songwriting-and-ai-mu…` would be compared as a name that
    # matches nothing while still looking like a healthy row.
    _sl="$(env COLUMNS=400 "$PF_SKILLS_CMD" -p "$pname" skills list 2>/dev/null)"
    _rows="$(printf '%s\n' "$_sl" \
             | awk -F'│' 'NF>=6 && $2 !~ /^[[:space:]]*Name[[:space:]]*$/ {c++} END{print c+0}')"
    _enabled="$(printf '%s\n' "$_sl" \
                | awk -F'│' 'NF>=6 {
                    gsub(/^[[:space:]]+|[[:space:]]+$/,"",$2);
                    gsub(/[[:space:]]/,"",$6);
                    if ($6 == "enabled" && $2 ~ /^[A-Za-z0-9._-]+$/) print $2 }' \
                | tr '\n' ' ')"

    _unexpected=""; _missing=""; _s=""
    for _s in $_enabled; do
      case " $_allow " in *" $_s "*) ;; *) _unexpected="$_unexpected $_s";; esac
    done
    for _s in $_allow; do
      case " $_enabled " in *" $_s "*) ;; *) _missing="$_missing $_s";; esac
    done
    _nu="$(printf '%s' "$_unexpected" | wc -w | tr -d ' ')"

    if [ "$_rows" = 0 ]; then
      # Not silence, and not a pass: zero rows means the enumeration or its
      # parse failed, which is indistinguishable from "nothing is enabled"
      # unless it is said out loud.
      warn "$pname: could not read which skills are enabled (no rows parsed from"
      say  "      '$PF_SKILLS_CMD -p $pname skills list'). NOT a pass."
    elif [ -n "$_unexpected" ] || [ -n "$_missing" ]; then
      warn "$pname: the enabled set does not match skills_allowed_for() (${_allow:-nothing allowed})"
      if [ -n "$_unexpected" ]; then
        _named=""
        for _a in $PF_ADJACENT; do
          case " $_unexpected " in *" $_a "*) _named="$_named $_a";; esac
        done
        _nn="$(printf '%s' "$_named" | wc -w | tr -d ' ')"
        say  "      $_nu enabled but not allowed, out of $_rows installed."
        # if/fi, not `cond && say`: the header comment on the helpers is not
        # decorative — every one of them returns 0 so that a `cond && x || y`
        # pair cannot fire both branches, and the way to keep that true is to
        # not build such pairs in the first place.
        if [ -n "$_named" ]; then
          say  "      acts in forge's own layers:$_named"
        fi
        if [ "$_nu" -gt "$_nn" ]; then
          say  "      + $((_nu - _nn)) other(s) not named here"
        fi
        say  "      skills.disabled is a snapshot of what was installed when this"
        say  "      profile was generated. Re-run ./hermes/profiles-bootstrap.sh."
      fi
      if [ -n "$_missing" ]; then
        say  "      allowed but NOT enabled:$_missing — skills_allowed_for() and the"
        say  "      live skills.disabled disagree. Re-run ./hermes/profiles-bootstrap.sh."
      fi
    else
      pass "$pname: enabled skills == allowlist (${_allow:-none}), $_rows installed"
    fi
  done
fi
