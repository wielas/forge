...
Coverage remains run-based, but telemetry and cost are session-based. When two
runs share the same `(profile, worker_session_id)`, JSON preserves both under
the session's `runs` mapping and names the tuple in `shared_attribution`; token
and cost totals charge that unique session once.
...
2026-08-11 after independent review found two runs sharing one session. They
are executable backfill, not memory; the earlier hand-authored rows remain
`n/a`.
